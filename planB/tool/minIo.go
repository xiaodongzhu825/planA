...
func UploadBase64Image(base64Data, fileName string) (string, error) {
	...
	// 构建完整对象名称（目录 + 文件名）
	// 确保目录格式正确（去掉首尾斜杠）
	targetDir := strings.Trim(golabl.Config.Minio.TargetDir, "/")
	var objectName string
	if targetDir != "" {
		objectName = targetDir + "/" + fileName
	} else {
		objectName = fileName
	}
	...
	// 上传到 MinIO
	uploadInfo, err := golabl.MinIo.Client.PutObject(
		context.Background(),
		golabl.Config.Minio.BucketName,
		objectName,
		reader,
		int64(len(imageData)),
	...
	imgUrl := golabl.Config.Minio.Url + "/" + golabl.Config.Minio.BucketName + "/" + uploadInfo.Key
